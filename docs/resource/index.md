---
title: 资源
date: 2025-04-30
category:
  - 资源
  - 工具
tag:
  - tools
---
# 资源

[[toc]]

## 常用搜索

+ [DeepSeek](https://chat.deepseek.com/)
+ [ 秘塔AI搜索](https://metaso.cn/)

## 技术博客

+ [Java 全栈知识体系](https://pdai.tech/)

## 技术社区

+ [Linux公社](https://www.linuxidc.com/)
+ [ stackoverflow](https://stackoverflow.com/)
+ [ segmentfault知否](https://segmentfault.com/)

## 代码托管

+ [GitHub](https://github.com/)
+ [Gitee](https://gitee.com/)
+ [CODING 腾讯云代码托管](https://coding.net/)

## 后端开发

+ [Spring Boot](https://spring.io/projects/spring-boot)
+ [ Mybatis](https://mybatis.org/mybatis-3/zh/index.html)
+ [ Mybatis Plus](https://mybatis.plus/)
+ [ Maven仓库](https://mvnrepository.com/)
+ [ Gradle](https://docs.gradle.org/current/userguide/userguide.html)

## 前端开发

+ [vuejs](https://cn.vuejs.org/)
+ [ bootcss](https://www.bootcss.com/)
+ [ npmjs](https://www.npmjs.cn/)
+ [ iviewui](https://www.iviewui.com/)
+ [ caniuse](https://caniuse.com/)

## CSS样式

+ [SASS](http://sass.bootcss.com/)
+ [ Stylus](https://stylus.bootcss.com/)
+ [ Less](https://less.bootcss.com/)
+ [ zeplin](https://app.zeplin.io/)
+ [ CSS triangle](http://apps.eky.hk/css-triangle-generator/)
+ [ CSS Arrow](http://www.cssarrowplease.com/)
+ [ Image Effects](https://bennettfeely.com/image-effects/)
+ [ clippy](http://bennettfeely.com/clippy/)

## 测试

+ [Reqable](https://reqable.com/zh-CN/)
  Reqable 是一款基于 Flutter 和 C++ 开发的跨平台 HTTP 调试与 API 测试工具，支持 HTTP/1、HTTP/2 和 HTTP/3（QUIC）协议，集抓包调试与接口测试于一体，提供高性能、轻量化的开发体验。

## 图片图标

+ [Font Awesome](http://fontawesome.dashgame.com/)
+ [ Squoosh 图片压缩](https://squoosh.app/)
+ [ 智图：腾讯图片压缩](http://zhitu.isux.us/)
+ [ Picdiet：在线批量压缩图片](https://www.picdiet.com/zh-cn)
+ [ 爱给：图片背景去除](http://www.aigei.com/bgremover)
+ [ Photopea：网页版Photoshop](https://www.photopea.com/)
+ [ iconfont：矢量图标](https://www.iconfont.cn/)
+ [ CSS ICON：CSS 图标](http://cssicon.space/)

## 工具合集

+ [菜鸟工具](https://c.runoob.com/)
+ [开源中国社区](https://tool.oschina.net/)
+ [tool.lu](https://tool.lu/)
+ [脚本之家](http://tools.jb51.net/)
+ [W3Cschool在线工具](https://123.w3cschool.cn/webtools)
+ [云转换](https://cloudconvert.com/)

## AI工具

+ [百度智能云千帆](https://cloud.baidu.com/product-s/qianfan_home)
+ [ChirpChirp](https://chirpchirp.cc)
  一个实用的 Copy&Paste AI Prompt 的小工具。它的核心功能是：搜索预先写好的提示，然后复制粘贴到需要的地方。
+ [提示词优化器](https://prompt.always200.com/)
  - [github仓库](https://github.com/linshenkx/prompt-optimizer)
  - [桌面应用下载地址](https://github.com/linshenkx/prompt-optimizer/releases)

## 插件

### 插件下载

+ [插件下载](https://www.chajianxw.com/product-tool/43487.html)

### 翻译插件

+ [会译对照式翻译](https://ad.huiyiai.net/site/huiyi2)
  准确度还可以，有免费额度
+ [沉浸式翻译](https://immersivetranslate.cn/)
  准确度不如会译对照式翻译

## 文档/建站工具

+ [VuePress](https://vuepress.vuejs.org/zh/)

## PDF处理

+ [Stirling-PDF](https://docs.stirlingpdf.com/)

## 其他资源

+ [30secondsofcode](https://30secondsofcode.org/)
+ [ jex.im/regulex：正则表达式可视化工具](https://jex.im/regulex/)
+ [ codepan](https://codepan.net/)
+ [ fiddle.md](https://fiddle.md/)
+ [ snipper](https://snipper.io/)
+ [ codesandbox](https://codesandbox.io/)
+ [ codrops](https://tympanus.net/codrops/)